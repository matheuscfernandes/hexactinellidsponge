...
DAll = np.array([1.])
LAll = DAll / 0.1
PoissonsRatioAll = np.array([0., 0.1, 0.2, 0.3])

for i in range(len(PoissonsRatioAll)):
    Mdb()
    PoissonsRatio = PoissonsRatioAll[i]
    spacing = LAll[0]
    x2 = spacing / (sqrt(2) + 2)
    rr = DAll[0] / 2.
    ...
    for THETA in THETAALL:
        ...
        file.write('%e %e %e %e %e %e %e %e\r\n' %
                   (Time, DAll[0], THETA, PoissonsRatio, DXX, DXY, DYX, DYY))
        file2.write('%e %e %e %e %e %e %e %e\r\n' %
                    (TimeS, DAll[0], THETA, PoissonsRatio, SXX, SXY, SYX, SYY))
