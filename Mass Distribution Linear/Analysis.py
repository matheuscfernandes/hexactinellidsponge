...
MASSFRACTIONALL = np.unique(np.append(np.linspace(
    0.01, 1, 10), 1. / np.linspace(1, 0.01, 10)))
...
for MassFraction in MASSFRACTIONALL:
    ...
    REDGES = ((VT * MassFraction) / (4. * pi * 2. *
                                     spacing * (MassFraction + 1.)))**(1. / 2.)
    RDIAGONALS = ((VT) / (4. * sqrt(2.) * pi * 2. *
                          spacing * (MassFraction + 1.)))**(1. / 2.)
