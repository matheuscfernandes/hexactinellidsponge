...
MASSFRACTIONALL=np.unique(np.append(np.linspace(0.01,1,10),1./np.linspace(1,0.01,10)))

x2=spacing/(sqrt(2)+2)

ct=-1
for MassFraction in MASSFRACTIONALL:
    Mdb()
    ct+=1
    ...
    REDGES=((VT*MassFraction)/(4.*pi*2.*spacing*(MassFraction+1.)))**(1./2.)
    RDIAGONALS=((VT)/(4.*sqrt(2.)*pi*2.*spacing*(MassFraction+1.)))**(1./2.)
