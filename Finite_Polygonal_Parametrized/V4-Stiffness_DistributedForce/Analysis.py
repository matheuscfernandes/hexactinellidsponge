...
def RunSimulation(NUMBEROFSIDES,NUMBEROFVERTICALSYSTEMS,NUMBEROFSYSTEMSPERSIDE,FileWrite):
    ...
    for i in mdb.models['Model-1'].rootAssembly.sets['ALL-VERTECIS'].nodes:
        if ((sqrt(i.coordinates[0]**2+i.coordinates[2]**2))>(RADIUS-TOL) and not i.coordinates[1]%spacing and i.coordinates[0]>0-TOL and i.coordinates[2]>RADIUS-TOL and i.coordinates[1]>0):
    ...
    mdb.models['Model-1'].ConcentratedForce(cf3=-LOAD, createStepName='Step-1',
