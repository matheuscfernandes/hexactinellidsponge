...
class Model(object):
    ...
    def __findDesign__(self,Design):
        if Design=='A':
            self.twoDiag=True;self.fullDiag=False;self.oneDiag=False
        if Design=='B':
            self.twoDiag=False;self.fullDiag=True;self.oneDiag=False
        if Design=='C':
            self.twoDiag=False;self.fullDiag=False;self.oneDiag=True
        if Design=='D':
            self.twoDiag=False;self.fullDiag=False;self.oneDiag=False
    def __dataAssertion__(self):
        # DATA VALIDATION FOR INPUT PARATMETERS/ WILL SPIT OUT ERROR IF SOMETHING IS NO CORRECT
        assert not self.NUMBEROFSIDES < 3,'Cannot have system with less than 3 sides: Problem not posed correctly!'

        assert not self.NUMBEROFSYSTEMSPERSIDE < 1,'Cannot have less than 1 cell in each system: Problem not posed correctly!'

        assert not self.NUMBEROFVERTICALSYSTEMS < 2,'Cannot have less than 2 systems high: Implemented code does not support!'


    def BuildGeometry(self,NUMBEROFSIDES,NUMBEROFVERTICALSYSTEMS,NUMBEROFSYSTEMSPERSIDE,FileWrite,Design):
        Mdb()
        NUMBEROFSYSTEMSPERSIDE=2*NUMBEROFSYSTEMSPERSIDE #conversion to UNIT-CELLs
        self.NUMBEROFSIDES=NUMBEROFSIDES
        self.NUMBEROFVERTICALSYSTEMS=NUMBEROFVERTICALSYSTEMS
        self.NUMBEROFSYSTEMSPERSIDE=NUMBEROFSYSTEMSPERSIDE
        self.__dataAssertion__()

        self.__findDesign__(Design)
        twoDiag=self.twoDiag
        fullDiag=self.fullDiag
        oneDiag=self.oneDiag

        spacing = self.spacing
        TOL = self.TOL
        DD = self.DD
        YoungsModulus = self.YoungsModulus
