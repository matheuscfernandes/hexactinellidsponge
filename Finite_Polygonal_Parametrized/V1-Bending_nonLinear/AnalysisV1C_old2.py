...
RADIUS = spacing * NUMBEROFSIDESPERSYSTEM / \
    (2 * math.sin(math.pi / (NUMBEROFSYSTEMS)))
ANGLE = 180 * (NUMBEROFSYSTEMS - 2.) / (NUMBEROFSYSTEMS)
...
assert(NUMBEROFSYSTEMS % 2 == 1 and NUMBEROFSIDESPERSYSTEM % 2 == 1,
       'Cannot have odd number of systems and odd number of sides in each system: Problem not posed correctly!')

assert(NUMBEROFSYSTEMS < 3,
       'Cannot have system with less than 3 sides: Problem not posed correctly!')

assert(NUMBEROFSIDESPERSYSTEM < 2,
       'Cannot have less than 2 sides in each system: Problem not posed correctly!')
...
if NUMBEROFSIDESPERSYSTEM % 2 == 1 and NUMBEROFSIDESPERSYSTEM > 1:
    ...
    mdb.models['Model-1'].rootAssembly.LinearInstancePattern(direction1=(1.0, 0.0,
                                                                         0.0), direction2=(0.0, 1.0, 0.0), instanceList=('Part-2-1', ), number1=(NUMBEROFSIDESPERSYSTEM + 1) / 2 - 1, number2=1, spacing1=spacing * 2, spacing2=spacing)
    ...
    mdb.models['Model-1'].rootAssembly.LinearInstancePattern(direction1=(1.0, 0.0,
                                                                         0.0), direction2=(0.0, 1.0, 0.0), instanceList=('Part-1-1', ), number1=(NUMBEROFSIDESPERSYSTEM + 1) / 2 - 1, number2=1, spacing1=spacing * 2, spacing2=spacing)
    mdb.models['Model-1'].rootAssembly.LinearInstancePattern(direction1=(1.0, 0.0,
                                                                         0.0), direction2=(0.0, 1.0, 0.0), instanceList=('Part-2-1', ), number1=(NUMBEROFSIDESPERSYSTEM + 1) / 2, number2=1, spacing1=spacing * 2, spacing2=spacing)
...
if NUMBEROFSYSTEMS % 2 == 1:
    mdb.models['Model-1'].rootAssembly.RadialInstancePattern(axis=(0.0, 1.0, 0.0),
                                                             instanceList=('Part-3-1', ), number=int(NUMBEROFSYSTEMS / 2. - 0.5), point=(0.0, 0.0, 0.0), totalAngle=360.0 * ((float(NUMBEROFSYSTEMS) - 3.) / float(NUMBEROFSYSTEMS)))

    mdb.models['Model-1'].rootAssembly.Instance(dependent=ON,
                                                name='Part-3-1-1', part=mdb.models['Model-1'].parts['Part-3-1'])

    mdb.models['Model-1'].rootAssembly.rotate(angle=-90, axisDirection=(
        0.0, 1.0, 0.0), axisPoint=(0.0, 0.0, 0.0), instanceList=('Part-3-1-1',))

    mdb.models['Model-1'].rootAssembly.translate(
        instanceList=('Part-3-1-1', ), vector=(0.0, 0.0, RADIUS))

    mdb.models['Model-1'].rootAssembly.rotate(angle=-90 + ANGLE, axisDirection=(
        0.0, 1.0, 0.0), axisPoint=(0.0, 0.0, RADIUS), instanceList=('Part-3-1-1',))

    mdb.models['Model-1'].rootAssembly.rotate(angle=360 *, axisDirection=(0.0, 1.0, 0.0), axisPoint=(0.0, 0.0, 0.0), instanceList=('Part-3-1-1',))


else:
    mdb.models['Model-1'].rootAssembly.RadialInstancePattern(axis=(0.0, 1.0, 0.0),
                                                             instanceList=('Part-3-1', ), number=NUMBEROFSYSTEMS / 2, point=(0.0, 0.0, 0.0), totalAngle=360.0)
...
mdb.models['Model-1'].rootAssembly.RadialInstancePattern(axis=(0.0, 1.0, 0.0),
                                                         instanceList=('Part-Top-1', ), number=NUMBEROFSYSTEMS, point=(0.0, 0.0, 0.0), totalAngle=360.0)
mdb.models['Model-1'].rootAssembly.InstanceFromBooleanMerge(domain=GEOMETRY,
