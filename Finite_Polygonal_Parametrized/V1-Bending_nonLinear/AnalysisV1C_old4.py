...
def RunSimulation(NUMBEROFSIDES,NUMBEROFVERTICALSYSTEMS,NUMBEROFSYSTEMSPERSIDE,FileWrite):
    ...
    assert not (NUMBEROFSIDES % 2 == 1 and NUMBEROFSYSTEMSPERSIDE % 2 == 1), 'Cannot have odd number of systems and odd number of sides in each system: Problem not posed correctly!'

    assert not NUMBEROFSIDES < 3,'Cannot have system with less than 3 sides: Problem not posed correctly!'

    assert not NUMBEROFSYSTEMSPERSIDE < 2,'Cannot have less than 2 sides in each system: Problem not posed correctly!'

    assert not NUMBEROFVERTICALSYSTEMS < 2,'Cannot have less than 2 systems high: Implemented code does not support!'

    # DEFINING PROFILE RADII - THIS IS FOR THE ROUND CROSSECTION
    if oneDiag:
        REDGES = DD/2.
    ...
    mdb.models['Model-1'].parts['Part-2'].SectionAssignment(offset=0.0,
                                                            offsetField='', offsetType=MIDDLE_SURFACE, region=mdb.models['Model-1'].parts['Part-2'].sets['EDGES'], sectionName='EDGES', thicknessAssignment=FROM_SECTION)
    mdb.models['Model-1'].parts['Part-2'].assignBeamSectionOrientation(method=N1_COSINES, n1=(
        1.0, 1.0, 1.0), region=mdb.models['Model-1'].parts['Part-2'].sets['EDGES'])
    ...
    if oneDiag or twoDiag or fullDiag:
        ...
        mdb.models['Model-1'].BeamSection(consistentMassMatrix=False, integration=DURING_ANALYSIS, material='Material-1', name='DIAGONAL', poissonRatio=0.0,
                                          profile='DIAGONAL', temperatureVar=LINEAR)
        mdb.models['Model-1'].parts['Part-2'].SectionAssignment(offset=0.0,
                                                                offsetField='', offsetType=MIDDLE_SURFACE, region=mdb.models['Model-1'].parts['Part-2'].sets['DIAGONAL'], sectionName='DIAGONAL', thicknessAssignment=FROM_SECTION)
        mdb.models['Model-1'].parts['Part-2'].assignBeamSectionOrientation(method=N1_COSINES, n1=(
            0.0, 0.0, -1.0), region=mdb.models['Model-1'].parts['Part-2'].sets['DIAGONAL'])
    ...
    Part_Full = mdb.models['Model-1'].parts['Part-Top']
    diagIndex = []
    for ii in range(len(Part_Full.edges)):
        diagIndex.append(Part_Full.edges[ii:ii + 1])
    ...
    mdb.models['Model-1'].BeamSection(consistentMassMatrix=False, integration=DURING_ANALYSIS, material='Material-1', name='EDGES', poissonRatio=0.0,
                                      profile='EDGES', temperatureVar=LINEAR)
    mdb.models['Model-1'].parts['Part-Top'].SectionAssignment(offset=0.0,
                                                            offsetField='', offsetType=MIDDLE_SURFACE, region=mdb.models['Model-1'].parts['Part-Top'].sets['EDGES'], sectionName='EDGES', thicknessAssignment=FROM_SECTION)
    mdb.models['Model-1'].parts['Part-Top'].assignBeamSectionOrientation(method=N1_COSINES, n1=(
        0.0, 0.0, 1.0), region=mdb.models['Model-1'].parts['Part-Top'].sets['EDGES'])
    ...
    mdb.models['Model-1'].rootAssembly.Set(name='TOP', nodes=Instance_Full.nodes.getByBoundingBox(-RADIUS - TOL, 2 * spacing * NUMBEROFVERTICALSYSTEMS - TOL, -RADIUS - TOL, RADIUS + TOL, 2 * spacing * NUMBEROFVERTICALSYSTEMS + TOL, RADIUS + TOL))
    ...
    mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, buckleCase=PERTURBATION_AND_BUCKLING, createStepName='Buckle',
                                         distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name='BC-TOP', region=Region(referencePoints=(
                                             mdb.models['Model-1'].rootAssembly.instances['REF-POINT-TOP'].referencePoints[1],
                                         )), u1=sqrt(2), u2=UNSET, u3=sqrt(2), ur1=UNSET, ur2=UNSET, ur3=UNSET)
    ...
    Values = [NUMBEROFSIDES, NUMBEROFSYSTEMSPERSIDE] + EigenValues

    DeleteAbaqusFiles(JobName)

    FileWrite.write(CreateEString(10) % tuple(Values))
